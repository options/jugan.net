...
### 금주의 패키지: JSON.NET 10
 JSON.NET에 대해서는 이미 지난 여러 코너(링크)에서 다룬 적이있습니다. JSON.NET은 NuGet 패키지(링크)로서 5억 이상의 다운로드를 기록하고있는  다운로드 1위 패키지로서 현재 버전10이 릴리즈되었습니다. 이번 릴리즈된 버전에는 다양한 기능이 추가되었으며 완전한 비동기 모드를 지원한다고 합니다.
